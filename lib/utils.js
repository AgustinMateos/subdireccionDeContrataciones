...
// Una renovación en trámite (o un parche cuya propia adjudicación se
// resolvió como fracasada/desierta — ej. una de las contrataciones que salió
// de dividir una convocatoria fracasada, que a su vez vuelve a fracasar) ya
// no está realmente "en curso" — no tiene sentido mostrar ni su estado como
// vigente/en trámite ni una fecha de vencimiento/inicio que ya no va a
// cumplirse. Solo renovación y parche llegan a tener un estadoConvocatoria
// fallido (lo asigna "Resolver adjudicación"), así que alcanza con el rol.
export function esConvocatoriaFracasada(exp) {
  return (exp.rol === "renovacion" || exp.rol === "parche") && ESTADOS_CONVOCATORIA_FALLIDOS.includes(exp.estadoConvocatoria);
}
...
// Igual que fechaMinimaRenovacion pero para una renovación UNIFICADA: la
// fecha de inicio no puede ser anterior a que termine la cobertura vigente
// de NINGUNA de las cadenas que se están uniendo, así que se toma la más
// tardía entre todas.
export function fechaMinimaUnificacion(origenes, expedientes) {
  return (origenes || [])
    .map(o => fechaMinimaRenovacion(o, expedientes))
    .filter(Boolean)
    .reduce((max, f) => (!max || new Date(f) > new Date(max) ? f : max), null);
}

// Día siguiente a una fecha "YYYY-MM-DD" (también "YYYY-MM-DD" + hora). Usado
// para proponer el inicio de una renovación: arranca el día después de que
// termina la cobertura vigente. Devuelve "" si no hay fecha.
...
// `creadoEn` y `Observacion.fecha` son timestamps reales (@default(now())),
// a diferencia de fechaInicio/fechaVencimiento/etc. que son fechas puras
// tipeadas por el usuario. Truncar su ISO string (que viene en UTC) a los
// primeros 10 caracteres da el día calendario de UTC, no el del usuario: en
// Argentina (UTC-3), cualquier registro después de las 21hs cae "al día
// siguiente" y los días frenado arrancan en -1. Acá se toma el día
// calendario según la hora local del navegador en su lugar.
export function soloFechaLocal(iso) {
  if (!iso) return "";
  const d = new Date(iso);
  return d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
}

// Para campos con hora (ej. fecha y hora de apertura), guardados/leídos como
// "YYYY-MM-DDTHH:mm" tal cual los tipeó el usuario (ver parseFechaHora).
export function fmtFechaHora(f) {
  if (!f) return "-";
  const [fecha, hora] = String(f).split("T");
  if (!hora) return fmtFecha(fecha);
  return fmtFecha(fecha) + " " + hora.slice(0, 5) + "hs";
}

// Un <input type="datetime-local"> entrega "YYYY-MM-DDTHH:mm" sin huso
// horario. Para que el valor guardado sea siempre el mismo que el tipeado
// (sin importar el huso del servidor), se lo interpreta explícitamente como
// UTC al guardar; como se vuelve a mostrar recortando el ISO string tal cual
// (ver normalizarExpediente), el round-trip devuelve el mismo texto.
export function parseFechaHora(v) {
  if (!v) return null;
  return v.includes("T") ? new Date(v + "Z") : new Date(v);
}
...
// Búsqueda de una orden de compra en el sistema de OC del PJN. `oc` es el
// texto tal como se cargó (ej. "67/2027" u "OC 67/2027"); se busca sin el
// prefijo "OC".
export function linkOrdenDeCompra(oc) {
  const numero = String(oc || "").trim().replace(/^OC\s*/i, "");
  if (!numero) return null;
  return "https://srpcm.pjn.gov.ar/ordenes_de_compra?filters[search]=" + encodeURIComponent(numero);
}

// Búsqueda del acta de apertura en el mismo sistema: se busca por el N° de
// contratación (ej. "22/2026").
export function linkActaApertura(nroContratacion) {
  const numero = String(nroContratacion || "").trim();
  if (!numero) return null;
  return "https://srpcm.pjn.gov.ar/actas_apertura?filters[search]=" + encodeURIComponent(numero);
}

// Separa un campo de OC que junta varias órdenes (ResolverAdjudicacion las
// une con " / ", con espacios — distinto de la barra de "67/2027").
export function ordenesDeCompra(texto) {
