...
// Fecha más temprana en que puede empezar la renovación de un vigente: no
...
export function fechaMinimaRenovacion(vigente, expedientes) {
  if (!vigente || vigente.rol !== "vigente") return null;
  const cadena = (expedientes || []).filter(e => e.cadenaId === vigente.cadenaId);
  const parches = cadena.filter(e => e.rol === "parche");
  return [vigente, ...parches]
